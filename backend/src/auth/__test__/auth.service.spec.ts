...
const nonActivatedUser: User = {
  ...
  hasId: () => true,
  save: () => Promise.resolve(nonActivatedUser),
  remove: () => Promise.resolve(nonActivatedUser),
...
describe("AuthService", () => {
  ...
  describe("login", () => {
    it("should return access_token if user exists and is activated", async () => {
      ...
      expect(await authService.login("email", "password")).toEqual({
        access_token: "sIgNeD",
      });
    });
    ...
    it("should throw if user is not activated", async () => {
      jest
        ...
        .mockResolvedValueOnce(nonActivatedUser);
    ...
    it("should throw if password is wrong", async () => {
      ...
      ).rejects.toThrowError("login.error.wrong_email_or_password");
    });
    it("should throw if no user found with email", async () => {
      jest.spyOn(userRepository, "findOne").mockResolvedValueOnce(undefined);
      await expect(authService.login("email", "password")).rejects.toThrowError(
        "login.error.wrong_email_or_password",
      );
    });
  });
  ...
  describe("signup", () => {
    it("should return user ", async () => {
      ...
      jest
        ...
        .mockReturnValueOnce(nonActivatedUser);
      await expect(
    ...
    it("should throw if user already xist ", async () => {
      ...
      await expect(
        authService.signup("email", "password"),
      ).rejects.toThrowError("signup.error.email_already_used");
    });
  });
  ...
  describe("activateUser", () => {
    it("should activate user ", async () => {
      ...
      jest
        ...
        .mockResolvedValueOnce(nonActivatedUser);
      expect(await authService.activateUser("email", "850018")).toEqual({
        ...nonActivatedUser,
        ...{ activated: true },
      });
    });
    ...
    it("should throw if code is incorrect ", async () => {
      jest
        ...
        .mockResolvedValueOnce(nonActivatedUser);
      await expect(
        authService.activateUser("email", "wrongCode"),
      ).rejects.toThrowError("activate.error.wrong_activation_code");
    });
